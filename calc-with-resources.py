...
def create_recipe_tree(
    ...
    for item in items:
        ...
        if curr_item_name not in resources:
            curr_resources[curr_item_name] = {
                "amount_required": 0,
                "amount_remaining": 0,
                "amount_created": 0,
            }

        have_amount = curr_resources[curr_item_name]["amount_remaining"]
        amount_remaining = have_amount - amount_required

        curr_resources[curr_item_name]["amount_required"] += amount_required

        if amount_remaining >= 0:
            curr_resources[curr_item_name]["amount_remaining"] = amount_remaining
            already_have_enough = True

        collected_resources.update(curr_resources)
        ...
        if found_recipes is not None:
            ...
            curr_ancestors = ancestors.copy()
            curr_ancestors.append(curr_item_name)
            ...
            for recipe_name in found_recipes:
                ...
                if not already_have_enough:
                    missing_amount = abs(amount_remaining)
                    recipe_multiplier = math.ceil(missing_amount / recipe_result_count)
                    amount_created = recipe_result_count * recipe_multiplier
                    curr_resources[curr_item_name]["amount_created"] = amount_created
                    curr_resources[curr_item_name]["amount_remaining"] = (
                        amount_created - missing_amount
                    )
                ...
                for ingredient in ingredient_list:
                    ...
                    for nested_ingredient in ingredients:
                        ...
                        new_recipe_tree, resource_tree = create_recipe_tree(
                            all_recipes,
                            all_item_tags,
                            supported_recipe_results,
                            items=[nested_ingredient],
                            ancestors=curr_ancestors,
                            resources=curr_resources,
                        )
                        ...
                        if (
                            ...
                            if new_recipe_tree.get("data") == curr_item_name:
                                ...
                                return new_recipe_tree, resource_tree
                    ...
                    collected_resources.update(resource_tree)
    ...
    return tree, collected_resources
...
def main():
    ...
    recipe_tree, collected_resources = create_recipe_tree(
    ...
    with open(output_file, "w") as write_file:
        json.dump(collected_resources, write_file, indent=4, sort_keys=False)
